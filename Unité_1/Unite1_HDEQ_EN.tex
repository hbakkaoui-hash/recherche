...
$\tau_0$). For time-independent $\Hsysop$,
\[
...
$\mathrm{Var}(\Delta\Phi(T_{\mathrm{dec}}))=1$.

\paragraph{(a) Quasi-static dispersion ($T_2^\ast$, refocusable).} If $\xi_i$ is
\emph{frozen} per constituent, the per-constituent dephasing grows linearly in $T$,
yielding a \emph{quadratic} variance
\[
...
\emph{re-fluctuates} cycle by cycle (a stochastic process), the variance becomes
\emph{linear} in $T$,
\[
...
$\lvert A_\psi(t)\rvert^2=1-(\Delta H_{\mathrm{eff}}/\hbar)^2 t^2+O(t^4)$, with
...
outcomes is taken as a heuristic postulate.
...
\begin{itemize}
\item $\tau_0$ and $\varepsilon$ are phenomenological, not derived from first principles (a geometric origin is conjectured, cf.\ conclusion).
...
\item The selection of a single branch is not resolved; the Born rule is heuristic.
\item The spectroscopic sector is non-discriminating under the consistent prescription (B).
\end{itemize}

\subsection{Origin of $\varepsilon$ and $\tau_0$: a geometric direction}
...
compactified-scale geometry --- a direction left to later development.
